...
async def channel_post(client: Client, message: Message):
    ...
    await reply_text.edit(f"<b>Here is Your Link : </b>{link}", reply_markup=reply_markup, disable_web_page_preview = True)
...
async def new_post(client: Client, message: Message):
    ...
    base64_string = await encode(string)
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Share URL", url=f'url={link}')]])
    try:
